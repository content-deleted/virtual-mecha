Shader "Jacob/ExplosionWeird"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				v2f o;
				o.y = v.vertex.y + v.vertex.x;
				/*float4 f = tex2Dlod(_DispTex, float4(v.uv.xy,0,0));

				float offset = _Displacement * _Displacement;

				v.vertex.xyz += v.normal*_ChannelFactor.rgb*f.rgb*offset;*/

				float3 dcolor = tex2Dlod (_DispTex, float4(v.uv.xy,0,0));
            	float d = 1.5 * (dcolor.r*_ChannelFactor.r + dcolor.g*_ChannelFactor.g + dcolor.b*_ChannelFactor.b);
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				half4 c = tex2D (_RampTex, float2(i.y/3 - 0.5,0.5));
				half4 b = tex2D (_RampTex, float2(d - 0.5,0.5));
				c+=(b-.5)/4;
				// apply fog
